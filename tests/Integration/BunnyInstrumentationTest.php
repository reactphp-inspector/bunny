...
final class BunnyInstrumentationTest extends AsyncTestCase
{
    ...
    public function publishWithoutArgsWorks(): void
    {
        ...
        try {
            ...
            /** @phpstan-ignore classConstant.deprecatedInterface */
            self::assertSame($routingKey, $span->getAttributes()->get(TraceAttributes::MESSAGING_DESTINATION_NAME));
            /** @phpstan-ignore classConstant.deprecatedInterface */
            self::assertSame($routingKey, $span->getAttributes()->get(TraceAttributes::MESSAGING_RABBITMQ_DESTINATION_ROUTING_KEY));
    ...
    public function channelMessageOperations(string $messageInteraction): void
    {
        ...
        try {
            ...
            /** @phpstan-ignore classConstant.deprecatedInterface */
            self::assertSame($routingKey, $publishSpan->getAttributes()->get(TraceAttributes::MESSAGING_DESTINATION_NAME));
            /** @phpstan-ignore classConstant.deprecatedInterface */
            self::assertSame($routingKey, $publishSpan->getAttributes()->get(TraceAttributes::MESSAGING_RABBITMQ_DESTINATION_ROUTING_KEY));
    ...
    private function getRabbitConnection(): ClientInterface
    {
        ...
        if (! is_string($url) || $url === '') {
            self::fail('Expected RabbitMQ connection URL to be a non-empty string');
        }
